...
export default function AuctionCard({ auction }: { auction: AuctionLike }) {
  ...
  const urgent = isListing(auction) ? new Date(auction.end_time).getTime() - Date.now() < 3 * 60 * 60 * 1000 : auction.urgent
  const currentBid = isListing(auction) ? auction.current_bid : auction.bid
  const endsLabel = isListing(auction) ? timeLeft(auction.end_time) : auction.endsIn
