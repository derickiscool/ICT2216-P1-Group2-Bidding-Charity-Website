...
export default function RegisterCharityPage() {
  ...
  // Registration flow: account form -> email OTP -> charity application submitted.
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // FR05 bugfix:
      // Charity Organisation accounts should not collect or send username.
      // Username remains applicable only for Donor/Bidder flows.
  ...
  const onVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Log in automatically so the charity application can be submitted with
      // an authenticated session and CSRF token.
      ...
      <div className="w-full max-w-xl">
        ...
        <div className="rounded-2xl px-8 py-8 shadow-sm" style={{ background: C.white, border: `1px solid ${C.beige}` }}>
          ...
          <form onSubmit={onSubmit} noValidate className="space-y-6">
            <div>
              <h3 className="text-sm font-bold uppercase tracking-wider mb-4 border-b pb-2" style={{ color: C.slate, borderColor: C.beige }}>
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      style={inputSt(!!errors.confirm, { paddingRight: '42px' })}
                    />
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full py-3 rounded-xl text-sm font-semibold text-white flex items-center justify-center gap-2 transition-all mt-4"
