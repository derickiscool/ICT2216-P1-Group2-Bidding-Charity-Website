...
WHERE NOT EXISTS (
    ...
    WHERE payment_ref IN ('DEMO-POCKET-WATCH-001', 'bfgref:' || encode(digest('DEMO-POCKET-WATCH-001', 'sha256'), 'hex'))
  )
...
WHERE NOT EXISTS (
    SELECT 1 FROM payments
    WHERE payment_ref IN ('DEMO-CERAMIC-VASE-001', 'bfgref:' || encode(digest('DEMO-CERAMIC-VASE-001', 'sha256'), 'hex'))
  )
