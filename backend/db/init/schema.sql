...
-- Migration: extend listing status to include post-sale fulfillment states
ALTER TABLE listings DROP CONSTRAINT IF EXISTS listings_status_check;
ALTER TABLE listings ADD CONSTRAINT listings_status_check
  CHECK (status IN ('draft', 'pending', 'active', 'sold', 'shipped', 'delivered', 'expired', 'cancelled', 'rejected'));

-- SFR14: digital donation receipts — immutable, no updated_at column
CREATE TABLE IF NOT EXISTS receipts (
  id               BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  uuid             UUID NOT NULL DEFAULT gen_random_uuid() UNIQUE,
  payment_id       BIGINT NOT NULL REFERENCES payments(id) ON DELETE RESTRICT UNIQUE,
...
CREATE INDEX IF NOT EXISTS receipts_listing_id_idx ON receipts (listing_id);

-- SFR15: shipping verification details provided by donor after payment
