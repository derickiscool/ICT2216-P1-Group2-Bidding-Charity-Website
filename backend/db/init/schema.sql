...
-- ─── NFSR04 / NFSR10 — WORM enforcement on audit_events ──────────────────────
-- These triggers make audit_events behave like Write-Once-Read-Many storage at
-- the database level, blocking any modification even from a DBA psql session.
-- TRUNCATE (used only by the test-reset helper) bypasses row-level triggers by
-- design — the test environment is not a production compliance boundary.

CREATE OR REPLACE FUNCTION audit_events_block_update()
RETURNS TRIGGER LANGUAGE plpgsql AS $$
BEGIN
  RAISE EXCEPTION
    'WORM violation: audit_events rows are immutable (NFSR04). UPDATE is not permitted.';
END;
$$;
...
-- NFSR10: minimum 365-day retention window — rows cannot be removed before
-- the compliance period has elapsed.
CREATE OR REPLACE FUNCTION audit_events_retention_check()
RETURNS TRIGGER LANGUAGE plpgsql AS $$
BEGIN
  IF OLD.timestamp > NOW() - INTERVAL '365 days' THEN
    RAISE EXCEPTION
      'Retention policy violation: audit_events rows must be retained for at least 365 days (NFSR10). Row timestamp: %', OLD.timestamp;
  END IF;
  RETURN OLD;
END;
$$;
