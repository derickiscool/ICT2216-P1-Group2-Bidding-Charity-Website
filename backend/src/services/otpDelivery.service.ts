...
/**
 * Test/development OTP handling
 * -----------------------------
 * In production, OTPs must be delivered by email.
 * In development/test, OTPs are stored in memory so automated tests can read
 * them without requiring SMTP/DKIM configuration. This prevents test runs from
 * failing when MAIL_DKIM_PRIVATE_KEY_PATH is not configured locally.
 */
...
export const sendPasswordChangeVerificationOtp = async (email: string, otp: string): Promise<void> => {
  if (!shouldSendRealEmail()) {
    ...
    console.info(`[BidForGood DEV PASSWORD CHANGE OTP] email=${email} otp=${otp}`);
    return;
  }
