...
import { addUser, findUserByEmail, findUserByUuid, getCharityByOwnerUserId, listStaffByCharityId, toPublicUser, updateUser } from '../repositories';
import type { PublicUser } from '../repositories';
import { badRequest, forbidden, notFound } from '../utils/errors';
import { isStrongPassword } from '../utils/breachedPasswords';
import { isValidEmail, safeString, sanitizeText, sha256 } from '../utils/security';
...
const systemUsernameForStaff = (email: string): string => {
  // Usernames are only user-facing for donor/bidder accounts. Staff accounts use
  // email for login and receive an internal username only to satisfy the existing
  // non-null database column without showing that value in the UI.
  return `staff_${sha256(email).slice(0, 24)}`;
};
...
export const createManagedStaff = async (req: Request): Promise<PublicUser> => {
  ...
  const user = await addUser({
    email,
    username: systemUsernameForStaff(email),
    full_name: fullName,
    roles: ['charity_staff'],
    passwordHash,
    is_verified: true,
  ...
  await audit(req, 'CHARITY_STAFF_CREATED', { email, mustChangePassword: true }, 'user', user.uuid, req.user!.id);
...
export const deactivateManagedStaff = async (req: Request, staffUuid: string): Promise<PublicUser> => {
  ...
  if (!target.is_active) return toPublicUser(target);
